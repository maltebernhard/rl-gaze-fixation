...
def plot_training_progress_multiple(callbacks: List[PlottingCallback], timestep, training_timesteps, observation_mode, reward_mode):
    ...
    if observation_mode == 1: obsmode = "angular"
    else: obsmode = "positional"

    if reward_mode ==1: rewmode = "without movement penalty"
    else: rewmode = "with movement penalty"

    # Add labels and title
    plt.xlabel('Episodes')
    plt.ylabel('Reward')
    plt.title(f'Training Progress for {training_timesteps} timesteps ({timestep} seconds each),\nwith {obsmode} observations and {rewmode}')
    plt.legend()
    plt.show()